...
export const createPlayMiddleware = initialPlaysObject => {
  ...
  const playMiddleware = store => next => action => {
    ...
    playDefinitions.forEach((playFunction, playName) => {
      ...
      const playResult = playFunction(action, StoreProxy(playInstance, store));

      Promise.resolve(playResult).finally(() => {
